...
El problema \textbf{Unbounded Knapsack (UK)} es una variante del problema clásico de la mochila (\textbf{Knapsack (K)}) en la cual los elementos pueden ser seleccionados múltiples veces. Para demostrar que \textbf{UK} es NP-duro, realizamos una reducción desde \textbf{UK} a \textbf{K}. La idea principal es transformar una instancia de \textbf{UK} en una instancia equivalente de \textbf{K}, resolviendo iterativamente subproblemas hasta obtener una solución completa.

\section*{Definición del Problema}
Dada una instancia de \textbf{UK} con:
\begin{itemize}
...
Por otro lado, el problema \textbf{K} es similar, pero cada elemento solo puede ser seleccionado una vez.

\section*{Reducción de UK a K}
La reducción se realiza mediante el siguiente algoritmo:

\begin{enumerate}
    \item Ejecutar el algoritmo de \textbf{UK} sobre la instancia original con capacidad $W$. Sea $S_1$ el conjunto de elementos seleccionados (sin repeticiones) y $P_1$ el peso total de estos elementos.
    
    \item Resolver un nuevo subproblema de \textbf{UK} utilizando todos los elementos excepto los de $S_1$, con una capacidad de $W - P_1$.
    
    \item Repetir este proceso iterativamente: en cada paso, resolver un subproblema de \textbf{UK} con los elementos restantes y la capacidad residual actualizada.
    
    \item El algoritmo termina cuando el subconjunto devuelto por el algoritmo	 es vacío.
\end{enumerate}

\section*{Demostración de Correctitud}
Demostramos que esta reducción es válida mostrando que la solución obtenida mediante el algoritmo descrito es equivalente a la solución óptima de \textbf{K}:

\begin{itemize}
    \item En cada iteración, el algoritmo selecciona un subconjunto de elementos que maximiza el valor dentro de la capacidad residual. Esto asegura que no se pierda ninguna combinación potencialmente óptima.
    
    \item Al eliminar los elementos seleccionados en cada iteración, garantizamos que las soluciones parciales no se superpongan, lo que simula el comportamiento de \textbf{UK}.
    
    \item Finalmente, cuando el algoritmo devuelve un conjunto vacío, se ha utilizado toda la capacidad disponible de la mochila, y la solución acumulada es óptima.
\end{itemize}

\section*{Conclusión}
Mediante esta reducción, hemos demostrado que cualquier instancia de \textbf{UK} puede ser transformada en una secuencia de instancias de \textbf{K}. Como \textbf{K} es un problema NP-completo, concluimos que \textbf{UK} es NP-duro.


	\chapter{Problema de Balanceo de Carga}
	...
	\subsection{Reducción desde Partition al problema de decisión}
	...
	Dado un conjunto de números $ \{p_1, p_2, \dots, p_n\} $, ¿es posible dividirlo en dos subconjuntos disjuntos $ A $ y $ B $ tales que:
	\[
	...
	\subsection{El problema de optimización NP duro}
